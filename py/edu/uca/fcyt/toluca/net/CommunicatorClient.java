...
public class CommunicatorClient extends Communicator
{
	...
	System.out.println("player: " + event.getPlayer() + ", carta: " + event.getCard());
	TrucoPlay tp;
	...
	if(event.getNumberOfHand() == TrucoEvent.ENVIAR_CARTAS)
	    ...
	    tp = new TrucoPlay(event.getPlayer(), TrucoEvent.JUGAR_CARTA, event.getCard());
	else
	{
	    new Exception("Aca posiblemente debe ir un cacho de _switch_ para cada jugada").printStackTrace(System.out);
	    tp = null;
	}
	if(tp != null)
	{
	    ...
	    System.out.println("No hay beleza para el TrucoPlay en :" + getClass().getName());
    }
